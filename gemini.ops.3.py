...
from typing import TYPE_CHECKING, Optional, Callable, Union, List, Tuple, Dict
...
if TYPE_CHECKING:
    # Define OperatorType for clarity in type hints (used internally)
    OperatorType = Callable[['Tensor', ...], 'Tensor']
    # HilbertState type hint for clarity where needed
    from .hilbert import HilbertState

# CRT Constants
PI = 3.14159265358979323846

# Default parameters for coefficient profiles and operators
...
def _ensure_complex_arithmetic(val1, val2, operation: str):
    ...
    if operation == 'add':
        ...
        # Add epsilon for division stability if needed, depends on context
        return val1 / val2
...
def alpha_profile(S: Union[float, Tensor], alpha0: Union[float, Tensor] = DEFAULT_ALPHA0,
...
def beta_profile(S: Union[float, Tensor], beta0: Union[float, Tensor] = DEFAULT_BETA0,
...
def gamma_profile(D_norm: Union[float, Tensor], gamma0: Union[float, Tensor] = DEFAULT_GAMMA0,
...
def epsilon_profile(proj_norm_sq: Union[float, Tensor], epsilon0: Union[float, Tensor] = DEFAULT_EPSILON0,
    ...
    def backward(ctx, grad_output: Tensor) -> Tuple[Optional[Tensor], ...]:
        ...
        if projections_ops:
            for proj_idx, proj_op in enumerate(projections_ops):
                ...
                # Need to handle non-linear P_k or P_k with own params carefully. Assuming linear for now.
                try:
...
class HarmonizationFunction(Function):
    ...
    def forward(ctx,
                ...
                # Parameters for internal S calculation (must use fixed parameters)
                ...
                # Parameters for internal D_norm calculation (must use fixed alpha)
...
def calculate_syntonic_stability(tensor: Tensor,
                                 # Params for D operator in S calc
                                 alpha_d: float = DEFAULT_ALPHA_FOR_S_CALC_D,
                                 d_projections: Optional[List[Callable]] = None,
                                 # Params for H operator in S calc (base coeffs)
                                 beta0_h: float = DEFAULT_BETA0_FOR_S_CALC_H,
                                 gamma0_h: float = DEFAULT_GAMMA0_FOR_S_CALC_H,
                                 # Params for H operator in S calc (profile params - fixed)
                                 kappa_h: float = DEFAULT_KAPPA_FOR_S_CALC_H,
                                 lambda_h: float = DEFAULT_LAMBDA_FOR_S_CALC_H,
                                 epsilon0_h: float = DEFAULT_EPSILON0_FOR_S_CALC_H,
                                 mu_h: float = DEFAULT_MU_FOR_S_CALC_H,
                                 # Ops for H operator in S calc
                                 h_projections: Optional[List[Callable]] = None,
                                 syntony_op_h: Optional[Callable[[Tensor], Tensor]] = None,
                                 # Params for D_norm calc within H in S calc (fixed)
                                 alpha_h_d_norm: float = DEFAULT_ALPHA_FOR_S_CALC_H_D_NORM,
                                 h_d_norm_projections: Optional[List[Callable]] = None
                                 ) -> Tensor:
    """
    Calculate the Syntonic Stability Index: S(Ψ) = 1 - ||D̂(Ψ)–Ĥ(Ψ)|| / ||D̂(Ψ)||.
    Uses fixed parameters for the internal D̂ and Ĥ operators.

    Args:
        tensor (Tensor): Input tensor Ψ.
        alpha_d (float): Alpha for the D operator used in this calculation.
        d_projections: Projections for the D operator used in this calculation.
        beta0_h, gamma0_h (float): Base coeffs for the H operator used in this calc.
        kappa_h, ..., mu_h (float): Fixed profile params for H used in this calc.
        h_projections: Projections for the H operator used in this calculation.
        syntony_op_h: Syntony operator for the H operator used in this calculation.
        alpha_h_d_norm: Alpha for D_norm calc within H used in this calc.
        h_d_norm_projections: Projections for D_norm calc within H used in this calc.

    Returns:
        Tensor: Scalar tensor representing the Syntonic Stability Index S(Ψ) [0, 1].
    """
    return SyntonicStabilityAutogradFunction.apply(
        tensor,
        alpha_d, beta0_h, gamma0_h,
        d_projections, h_projections, syntony_op_h,
        kappa_h, epsilon0_h, mu_h, lambda_h,
        alpha_h_d_norm, h_d_norm_projections
    )

# Alias for syntonic_stability
syntonic_stability = calculate_syntonic_stability

# --------------------------------------
# Fractal and Advanced Operators (Merged)
# --------------------------------------

def fractal_dimension(tensor: Tensor, min_box_size: int = 2,
                      max_box_size: Optional[int] = None) -> float:
    """
    ...
    for box_size in range(min_box_size, max_box_size + 1):
        ...
        if len(tensor.shape) == 0: # Scalar
            ...
            # Simplified: Iterate through all possible top-left corners of boxes
            import itertools
            ...
            for start_indices in itertools.product(*ranges):
                ...
                if box_has_value:
                    count +=1
            # raise NotImplementedError("Fractal dimension for >2D is complex and may be slow.")
...
H = harmonization
R = recursion
# syntonic_stability alias already defined above

# === END OF FILE merged_ops.py ===